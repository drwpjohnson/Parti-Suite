function HAP_hetdomain_infop()
% tile size
tile = 1;
% view area size
dom =tile/2;
% hetdomain sizes
rhet0 = tile/20; % large
rhet1 = rhet0/2; % medium
rhet2 = rhet1/2; % small
% unit circle
[xu,yu,zu]=HAP_info_circle(0,0,0,1.0,50);
figure('name','Example heterodomain tiles','NumberTitle','off');
%
subplot(1,2,1)
fill(xu*rhet0,yu*rhet0,'g');
hold on
title({'Large only',' (HETMODE = 1)'})
sgtitle({'Colloid Hetmode Options',...
         'Large/Small hetdomain and Fractional surface coverage',...
         'have error catching, accepted values are valid',...
         'for simulations lacking colloid rotation'}) 

axis([-dom dom  -dom dom]);
axis square
%
subplot(1,2,2)
fill(xu*rhet0,yu*rhet0,'g');
hold on
title({'1 Large, 4 Medium',' (HETMODE = 5)'})
% loop surrounding hetdomains
for i=1:4
    ang = i*pi/2;
    x = xu*rhet1+tile/3*cos(ang);
    y = yu*rhet1+tile/3*sin(ang);
    fill(x,y,'g');
end
axis([-dom dom  -dom dom]);
axis square
%


end