...
function [FVDW,FVDW2,Hasp_colloid,Hasp_domain]=AFMFORCEVDW (X,Y,Z,A132,AG,AP,ASPcolloid,ASPdomain,NASP,RMODE,xcap,ycap,zcap,xasp_domain,yasp_domain,zasp_domain,H,LAMBDAVDW,A11,...
...
if (VDWMODE==1)
    ...
    if (RMODE==2)   %ASPERITIES ON domain
        ...
        H2 = H + ASPdomain; %SEPARATION DISTANCE BETWEEN UNDERLYING SMOOTH COLLECTOR AND COLLOID
        %               CALCULATE VDW INTERACTION BETWEEN COLLOID AND COLLECTOR (SPHERE-SPHERE)
        FVDW1 = -(A132*AEFF/(6*H2^2.0))...
            *(LAMBDAVDW/(LAMBDAVDW+5.32*H2));
    ...
    if (RMODE==3)    %ASPERITIES ON BOTH SURFACES
        ...
        % loop through one colloid asperity interacting with all domain
        % asperities in ZOI
        ...
        for i=1:length(xcap)
            ...
            ycol = ycap(i);
            zcol = zcap(i);
            Harray = ((xcol-xasp_domain).*(xcol-xasp_domain)+(ycol-yasp_domain).*(ycol-yasp_domain)+(zcol-zasp_domain).*(zcol-zasp_domain)).^0.5-ASPcolloid-ASPdomain;  
            % find minimum separaion distance

            
            % array of total interaction of a single colloid asperity with
            % all domain asperities 
            FVDWoneasp = -(A132*AEFFASP./(6*Harray.^2.0))...
                .*(LAMBDAVDW./(LAMBDAVDW+5.32*Harray));
            % array of distances between colloid asperity center and domain asperities centers
            RXYZ = ((xcol-xasp_domain).*(xcol-xasp_domain)+(ycol-yasp_domain).*(ycol-yasp_domain) + (zcol-zasp_domain).*(zcol-zasp_domain)).^0.5;
            ...
            unity = (ycol-yasp_domain)./RXYZ;
            % Correct array of interactions to component on Y axis
            FVDW2(i) = sum(unity.*FVDWoneasp);
        end
        %  CALCULATE TOTAL INTERACTION
        FVDW = FVDW1 + sum(FVDW2);
    end
