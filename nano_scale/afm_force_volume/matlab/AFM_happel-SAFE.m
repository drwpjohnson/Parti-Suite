...
global XINITV YINITV Ybar Ypri centersBar centersPri
...
%% hardwire AG  for AFM mode as a 10000 factor of AP
AG = 1e9*AP;
...
if (A132<0.0&&GAMMA0AB>0.0)&&(ZETACST*ZETAPST>0.0)
    ...
    while (FCOLL>0.0)
        ...
        FEDL =AFMFORCEEDL (KAPPA,KB,ERE0,T,ZI,ECHG,ZETACST,ZETAPST,AG,AP,ASP,NASP0,RMODE,H,PI);
        FAB= AFMFORCEAB (PI,AG,AP,ASP0,NASP0,RMODE0,LAMBDAAB,GAMMA0AB,H,H0);
        FBORN=AFMFORCEBORN (A132,SIGMAC,AP,H,A11,A22,A33,AC1C1,AC2C2,VDWMODE);
        if (FVDW>0.0)
            FVDW = 0.0;
...
if RMODE==3 %roughnes on both surfaces colloid and collector
    %0.5*(2*ASP+3^0.5*ASP) TERM IS AVERAGE SEPARATION DISTANCE BETWEEN UNDERLYING
    %SMOOTH SURFACES FOR OPPOSED AND COMPLIMENTARY ASPERITY PACKING
    ZMIN = HMIN+AG+AP+0.5*(2*ASP+3^0.5*ASP);
    Hlow = HMIN+2*ASP;
    Hhigh = Hlow+HSPAN;
    ZMAX = ZMIN+HSPAN;
end
% create locations vector in Z
HVECTOR(1)=Hhigh;
i=1;
...
if ugrid ==0
    for j=1:NPARTLOOP
        [XINITV(j),YINITV(j),RINJV(j)]=AFMINITIAL (RLIM);
    end
end
% uniform probe locations (square)
if ugrid==1 
    xlim = linspace(-RLIM/2,RLIM/2,NPART); 
    ylim = linspace(-RLIM/2,RLIM/2,NPART); 
    [xloc,yloc]=meshgrid(xlim,ylim);
    [nr,nc]=size(xloc);
    j=1;
    for im=1:nr
        for jm=1:nc
    ...
    RINJV = (XINITV.*XINITV+YINITV.*YINITV).^0.5;
end
...
for J = 1:NPARTLOOP
    ...
    if RMODE==3 %roughnes on both surfaces colloid and collector
        ...
        %SMOOTH SURFACES FOR OPPOSED AND COMPLIMENTARY ASPERITY PACKING
        RINIT = (XINIT^2.0 + YINIT^2.0 + ZMAX^2.0)^0.5;
        HINIT = RINIT-AP-AG-0.5*(2*ASP+3^0.5*ASP);
        ZINIT = ZMAX;
    end
    
    %% calculate H array locations corresponding to ZVECTOR
    if RMODE==0 %smooth surfaces
        ZVECTOR = HVECTOR+AP+AG;
    end
    if RMODE==1||2 %roughnes on one surface colloid(1) or collector(2)
        ZVECTOR = HVECTOR+AP+AG+ASP;
    end
    if RMODE==3 %roughnes on both surfaces colloid and collector
        ZVECTOR = HVECTOR+AP+AG+0.5*(2*ASP+3^0.5*ASP);
    end
    ...
    for i=1:nsteps
        ...
        if H<=nsplotH&&H>2*HFRIC&&detplot==1&&(PCOUNT2==NSplotVAR||NSplot==1)
            ...
            subp=[1,2,1];
            [XHET,YHET,ZHET,RHET]=AFMHETTRACK(X,Y,Z,Xm0,Ym0,Zm0,AG,HETMODE,SCOV,RHET0,RHET1,RHET2,1);
        ...
        %             H = R - AG - AP + DELTA - ASP;
        %         elseif (RMODE==3)  %ROUGH-ROUGH INTERACTION
        %THIRD TERM IS AVERAGE SEPARATION DISTANCE BETWEEN UNDERLYING
        %SMOOTH SURFACES FOR OPPOSED AND COMPLIMENTARY ASPERITY PACKING
        %             H = R - AG - AP + DELTA - 0.5*(2*ASP+3^0.5*ASP);
        %         end
...
[maxFraw,imaxF] = max(FCOLLOT);
[minFraw,iminF] = min(FCOLLOT);
% check max and min sign and generate logic arrays
csignmax=maxFraw>0.0;
csignmin=minFraw<0.0;
% assign max and min separation location 
hmindisc = 1.0e-8; % upper bound primary force minimum
hmaxdisc1 = 1.0e-9; % lower bound force barrier
hmaxdisc2 = 5.0e-8; % upper bound force barrier
...
for i=1:NPARTLOOP
    % identify <hmindisc locations for minima
    if(HOT(iminF(i),i)<hmindisc)
        chmin(i)=true;
    else
        chmin(i)=false;
    end
    % identify >hmaxdisc1  and <hmaxdisc2 locations for maxima
    if(HOT(imaxF(i),i)>hmaxdisc1&&HOT(imaxF(i),i)<hmaxdisc2)
        chmax(i)=true;
    else
        chmax(i)=false;
    end
...
% convert heatmap arrays to 2D probe locations (xloc and yloc) 
[nr,nc]=size(xloc);
...
barFloc=NaN(nr,nc); %*maxinitF;
priFloc=NaN(nr,nc);%*mininitF;
for i=1:nr
    for j=1:nc
        % populate max force map location
        barFloc(i,j)= barFdisc(k); 
        % override if NaN force value is registered
%         if isnan(barFloc(i,j))
%             barFloc(i,j)=maxinitF;
%         end
        % populate min force map location
        priFloc(i,j)= priFdisc(k);
        % override if NaN force value is registered
%         if isnan(priFloc(i,j))
%             priFloc(i,j)=mininitF;
%         end  
...
figure(1)
% plot(xaux,yaux,'sb')
plot3(xaux,yaux,afaux,'sb')
title('Probe Locations')
axis square
hold on
% plot zoi around probe locations
for i=1:NPARTLOOP
    [xc,yc,zc]=AFMcircle(xaux(i),yaux(i),afaux(i),rzoiaux(i),50);
    plot3(xc,yc,zc,'-','Color',colcolor)
end
% plot hetdomains on AFMdomain
for j=1:NPARTLOOP
    for i=1:rangeHetV(j)
        [xc,yc,zc]=AFMcircle(mxhetOUT(i,j),myhetOUT(i,j),0.0,mrhetOUT(i,j),50);
        plot3(xc,yc,zc,'-g')
    end
end
...
%% maxforce heatmap
%% use image for 20x20, check zoom in for images, 
%% generate heterogeneity..specially in the pole to diagnose
% barrier heatmap
figure(3)
surf(xloc,yloc,barFloc,'EdgeColor','none')
...
figure(4)
surf(xloc,yloc,priFloc,'EdgeColor','none')
xlabel('X(m)'); ylabel('Y(m)'); zlabel('F(N)'); 
